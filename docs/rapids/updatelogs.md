...
- 现在近战攻击将能在空挥时照常击出范围攻击。
- 修复了在使用法杖时偶尔会触发近战攻击的问题。
- 新增面对面交易功能，你现可通过 `/trade <ID>` 与同一区服且距离 `8` 格内的玩家进行面对面交易。
- 新增钓鱼系统，你现可在打造台中通过低廉的价格制作钓鱼竿。
...
- **熔铸台** 相关功能：
  - 高品质装备于所能提升的数值有所提升：
    ...
    - `神话`: 
      - 单件: 7.5% → `20%`
      - 每 3 件额外增益：2.5% → `5%`
  - 你现可通过同 **套装** 之部分装备部件互相对装备进行熔铸：
    <br>经由不同部件所能提升的数值仅为同部件的 `50%`，且无法获得多件堆叠额外增益。
    <br>同部件与不同部件**可以堆叠**熔铸，并会依照各个部件的提升效果进行叠加。
    - 武器组：
      - `剑`
      - `弓`
      - `法杖`
...
- **晦渊司祭** 技能修改：
  - `渊门逐浪` 追加第四次大范围攻击。
    <br>前三浪释放后停滞更长时间，而后对 `16~32` 格内玩家造成一次 `250%` 伤害魔法攻击。
- **制式符文** 相关：
  - 将 `不屈的凯旋颂` 中 `生命恢复` 的部分属性转为 `会心` 及 `暴击伤害`，以增强近战核心符文的DPS提升职能。
  - 新增生命值相关职能附属符文： `长生的安魂谣` 。
- **近战攻击系数**调整：1.2 → `1.3`
- **如影随形**属性调整：
  - 元素属性：`暗属性` → `地属性`
- 以上涉及的属性调整已有装备会自动更新。
- Rank 特权对应的 US 扩容已实装。
- 修复了「峡谷」Essence 偶尔会出现无掉落的问题。
- 补全了 Tab 中各区服在线人数及当前所处区服的显示。

---

#### 2026/07/21

- 通过更完善的方法移除了近战攻击后的怪物受击冷却，现近战攻击将不会被客户端受击冷却影响以致攻速不满足实际期望。
- 你现在可以通过 `/ghost` 屏蔽其他服务器的玩家。
- 现在通过「精雕细琢」打造的 `史诗` 装备不再绑定。
- 新增了 **打造** 功能的保底计数：
  <br>将打造产出装备品质简略划分为 `A` `B` `C`，以常规为例即 `卓越`、`史诗`、`传奇`。
  - 打造产出 `A` 品质，将累计 `1` 点 **铸运值**。
  - 打造产出 `B` 品质，不累计 **铸运值**。
  - 打造产出 `C` 品质，重置 **铸运值**。
  - **铸运值** 累计至 `20` 点，必定获得一次 `C` 品质的装备。
- 修复了 `戒指` `项链` 可强化、升阶的问题。（实际无效）
- 提高了 Final 怪物的材料掉率：
  - `晦渊司祭`：
    - <span class="item-name-tag">归途残铃</span>: `8%` → `10%`
- 新增 Rank 特权 `宝箱立即搜掠` 功能，开启后将在右键宝箱时直接领取所有物资。
- 物品管理器新增耐久值的相关方法。
- 完成了随机事件运行的基本设计。
- 优化了钓鱼系统的部分内容。
- 优化了宝箱系统的部分内容。
...
- 重新制作了召唤怪物的方法，现应已能不会出现召唤异常以致怪物消失。
- 移除了怪物的受击冷却，现在在单一怪物受到多人攻击时应能正常计算伤害。
- 实现了更好的跨服消息通信及公告功能。
  - 现在打造产出的高品质装备将发送全服公告。
- 重设了更符合收益曲线的升阶需求公式，并降低了高品质需求倍率：
  - 需求倍率：
    - `传奇`: 8 → `6.4`
    - `神话`: 24 → `16`
  - 在这一调整后，部分等级的升阶需求将下降至原先的约 `30%`，（即旧需求为 3+ 倍）
- 怪物共享击杀需求调整：
  - `Essence` 怪物：10% → `8%`
  - `Final` 怪物：10% → `5%`
- 调整了 `怪物刷新护符` 的使用限额：
...
- 完成了随机事件的基本设计。
- 完成了统一的公告推送方法。
- 现在绑定的装备将在装备信息中显示 `灵魂牵引`。
- 修复了在某些情况下非「精雕细琢」产物也会被绑定的问题。
- 修复了部分问题。
- **制式符文** 相关：
  - 新增防御相关职能附属符文： `坚城的镇魂歌` 。
...
- 将 `苍风琉砂` `归途残铃` 调整为 `传奇` 品质。
- 提高了 Final 怪物的材料掉率：
  - `旧骨`：
    - <span class="item-name-tag">苍风琉砂</span>: `5%` → `8%`
  - `洞渊渡者`：
    - <span class="item-name-tag">归途残铃</span>: `5%` → `8%`
  - `晦渊司祭`：
    - <span class="item-name-tag">归途残铃</span>: `6.25%` → `8%`
- 你现可从 「河隘」 Final 怪物处获得更高品质的 `戒指` `项链`。
- 新增了 Rank 菜单，你可于此快捷进行部分交互方块的使用。并：
  - 使用 `怪物刷新护符`，以提升短时间内所处场景的怪物刷新速率。
- 调整了部分视觉效果的可见性范围。
- 修复了部分「河隘」天赋描述错误的问题。
- 修复了部分方法在遇到全数字 ID 时会出现异常的问题。
- 修复了部分其他问题。
...
- 重构了「宝箱搜寻器」的实现方式，得益于 HDLib 的功能提升，现已支持更直观，高可用性的宝箱位置指示。
  <br><img src="/rapids/rapids_img/updatelogs/20260714_lootchestdetector.png" loading="lazy">
- 削弱了「河隘」中的怪物属性。
  - **河隘僵尸** `Common`:
    - 生命值: `3.6` → `3.2` unit
    - 攻击力: `1.0 ~ 1.2` → `0.8 ~ 1.2` unit
  - **河隘射手** `Common`:
    - 生命值: `2.8` → `2.5` unit
  - **岩隘守尸** `Elite`:
    - 攻击力: `1.75 ~ 2.0` → `1.6 ~ 2.0` unit
  - **河雾怨魄** `Essence`:
    - 生命值: `20` → `17.5` unit
    - 攻击力: `4` → `3 ~ 3.5` unit
    - 攻击属性: `无属性` → `暗属性`
...
- 新的 `回忆` 设定，你需要在当副本中通过各渠道积累 `回忆` 才能解锁下一副本。
  - 因测试期间特殊性，早期副本暂不拟补设，仍全面开放，而 「河隘」`破晓` 及之后副本将正常需求。
- 副本「河隘」 `破晓` 现已开放。你需要于 `踏察` 中累计足够多的回忆才能解锁。
- 副本「河隘」 `踏察` 现已开放。
- 在以上副本内新增了大量宝箱及水晶。
- 完成了全新的独立的托管式饰品栏及持久化适配。
- 完成了完整的跨服聊天支持。
- 完成了新手引导关卡的设计。
- 完善了物品管理器的功能。
...
- 副本「河隘」`见闻` 现已开放。
  - 新增高危区域 `高台`，并附可召唤 Essence 的 `追忆点`。
  - 考虑到河隘地图较大，新增了中途撤离点。
  - 新增了部分宝箱。
- **再入**相关：
  - 调整了部分已有增益的数值。
  - Tier 1 再入增益新增 `生命恢复` `生命值`。
- 降低了天赋「搜略本能」和「宝藏眷顾」各阶段的价格。
- 新增了天赋「死亡回溯」，能让你在局内死亡时再次于局内复活。
- 「河隘」新增了 Tier 2 宝箱。
- 完善了「河雾」套装效果。
- 优化了部分交互方块/按钮/内容的描述。
- 优化了副本运行时数据结构。
