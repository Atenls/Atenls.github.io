...
- 自动拆解相关：
  - 现已升级为自动分解/拆解，你可以在设置中自行设置。
  - 修复了对于价值过低的物品无法自动拆解的问题，现在它们将被拆解为 `晶髓点数`，并在累计至足够兑换的点数后自动兑换。
-  `死魇` 难度奖励提升：
  - 提高了通关时附带的奖励 `E` 值；并将所有副本的数值统一为 `65` 级。
  - 在死魇难度下，选择 `通用补给` 时获得的 `E` 值现将翻倍。
...
- 修复了 `搜掠本能` 未对部分个人收集物生效的问题。
- 现在 Rank 用户使用扫荡时将直接离开副本。
- 部分细节调整和问题修复。
...
- 新增天赋 `珍藏追猎`：
  ...
  - 在中高级时，还将需求 [Rank](rapids/rank) 等级。
    <br>在 Rank 过期 / 降级后，最高可用等级将回退至当前可用的最高等级。
- 新增天赋 `宝箱磁铁`：
...
- 新增 `属性天赋`：
  <br><img src="/rapids/rapids_img/updatelogs/20260820_attrsmastery.png" class="dp-image" loading="lazy">
  - **支援槽位**，每个支援槽位可装配一项 `属性天赋`，通过副本金色材料解锁，解锁价格：1 / 8 / 64 / 512x。
  - **属性天赋**，目前共有五项属性天赋：
    - **基础**：同时提升 `会心` `柔韧` `命中` `闪避`，最高 Tier 6。
    - **攻击**：分为三系攻击，提升各对应的类型攻击，最高 Tier 4。
    - **元素**：提升本副本对应元素的属性强化及抗性，最高 Tier 4。
  - 属性天赋通过材料升级，材料使用混合扣除，可以阶段性提交。
    <br>材料价值跟随材料品质，你可以在 `材料选择` 中查看各材料对应价值，并决定是否启用对应材料品质。
  - 不同副本的属性天赋数值将基于本副本等级，且**跨副本全局生效**。
  - 目前为 Beta 测试版，后续仍有可能进行部分底层修改。
- 现在修改 `熟能生巧` 天赋启用等级将会默认停留在菜单。
- 修复了部分问题。
...
- 进行了部分交互方块美化。
- 在 `结算房间` 增加了 `拆解台`。
- 修复了 `谷垣`、`空庭` 套装部分部件词条数量未正常生效的问题。

---

#### 2026/08/15

- 完成了 `快速搜掠` 功能/天赋的部分前置技术。
- 你现在可以在宝箱中搜掠到 `回忆`，如下个难度/副本回忆已完成解锁，将获得本副本/难度的回忆点数。
- 修复了部分问题。
- 完成了部分技术内容的前置工作。
...
- 调整了装备事务需求 `E` 值及 `晶髓碎片` 的品质修正系数：（影响范围如：`升阶`、`重构`、`强化` 等）
  - `史诗`：+6.67%
  - `神话`：-20%
- 你现已可于 `装备重构台` 重构你的装备。
- 新的天赋 `库藏充盈`，你可通过 `E` 值来提升对应副本的 QS 最大容量。
  <br><img src="/rapids/rapids_img/updatelogs/20260813_qsmastery.png" class="dp-image" loading="lazy">
- 修复了副本结算奖励中，材料选项在 DM 难度的部分问题，并优化了多次领取时的材料聚合。
- 修复了部分问题。
...
- 对装备信息显示进行了更完善的修改，你现在可以自由选择是否查看装备的**详细信息**。
  ...
  - 通过按 `F` (副手键) 来变更显示模式。（基于此，你现在无法在打开容器界面时通过此按键来进行其余交互）
  - 在收起详细信息情况下，会隐藏套装效果、技能效果、灵魂牵引、重构次数以及副词条。
  - 现在重复的副词条会合并显示。
- 降低了 `雷尘伏蝠` 的攻击频率及攻击触发概率，综合下降约 `20%`。
- <span class="item-name-tag">Noctambulist</span> 或以上 Rank 现已可使用彩色字体。
  - 除原版提供的彩色代码外，你也可以通过 `&#rrggbb` 来使用RGB颜色；或通过 `&<#rrggbb>#<#rrggbb>` 来使用渐变色。
- 聊天系统使用了更完善的通信结构。
- 完善了封禁系统。
...
- 熔铸台现为灵魂牵引装备熔铸时，可通过次一等品质的装备进行熔铸。
  - 参考：预提升装备品质为 `神话`，可以消耗 `传奇` 品质的装备提升。惟提升数值仅原品质之 50%。
- 主城广场旁小屋新增符文镶嵌台，你现在可以为你的 `核心符文` 镶嵌 `附属符文`。
- 调整了部分异常的符文属性，使其现基于 Rapids 数值公式进行计算。
- 大幅度降低了 `雷尘伏蝠` 的闪避数值。
- 新增天赋 `血色猎获`，通过降低自身伤害、提高自身受到的伤害来提升局内装备产出收益。
- 提升了高难度怪物（Essence、Final）掉落装备的随机品质范围。
- 你现可在主城使用 `脱离卡死`。
- 修复了部分问题。
- 部分其他内容推进。
...
- 修复了在套装属性含负数最大生命时，切换装备（即使是同类型套装）也会触发血量抖动的问题。
- 修复了重置时不会恢复血量的问题。
- 将 `治疗药剂` 的堆叠上限修改为 `99`。（法力药剂保持为 `16`）
- 基础的 `US` 储存空间将从 `27` 扩展至 `54` Slots。
- 调整了元素属性容量的分配。
  <br>目前**武器副词条**含较高容量的元素抗性，使得武器的重要性远超装备，加剧了配装对武器的依赖。
  <br>为此，**削减了武器**所含副词条的元素抗性，并等比**提高了装备**所含副词条的元素抗性。
- 完成了跟追版本等级的长线难度 `DeathMare · 死魇` 之基础建设。
- 调整了副本中掉落随机装备的产出分布，并将在掉落时使用正确的品质染色。
- 通关奖励新增了 `通用材料` `饰品` 选项。
- 修复了部分问题。
...
- 调整了回忆进度的获取速率，现在在非版本当期的副本中，回忆进度获取速率将变得更快。

---

#### 2026/08/05

- 调整了命中率/闪避率公式，主要修改了命中高额溢出后的处理方式，旨在解决命中率无法达到 100% 的问题，并提升游戏体验。
  <br>在攻击者命中超过受击者闪避 `300%` 时，将开始线性增加命中率。直至 `600%` 后达到 `100%` 命中率。
  <br>参考图例：（橙色线为旧公式，蓝色线为新公式）
  <br><img src="/rapids/rapids_img/updatelogs/20260805_hitchance.png" loading="lazy">
    - 攻击者：命中 0 ~ 10800
    - 受击者：`雷尘伏蝠` | 等级 Lv.60 | 闪避：1644
- 添加了交易所的 `筛选器` 功能。
- **强化**相关：
  - 调整了强化价格的曲线，降低了较低强化等级时的价格，提高了较高等级时的价格。
  - 强化等级开放至 `+15`，注意：**强化等级超过 +12 后，失败时装备将会损毁！**
- 降低了主城宝箱产出的 `E` 值至合理水平，并将对应等级由玩家自身等级调整至**玩家所通关副本的最高等级**。
- 新增「主城宝箱 Tier II」，刷新时间 `2` hrs，有概率产出 `强化保护石`
...
- 修复了 `震垣蛮牛` 攻击属性不生效的问题。
- 添加了装备仓库的 `筛选器` 功能，你可以调整对应筛选器以只显示**同时满足**对应要求的装备。
  <br>目前支持分类：
  - 类型
  - 品质
  - 套装
...
- 补充了再入次数的信息披露。
- 修复了再入属性异常。
- 熟能生巧现将可自由调配启用等级，更新后你将需要自行重新设置启用等级。
- `Final` 怪物所掉落的材料将会额外显示于聊天栏。

---

#### 2026/08/02

- 调整了等级提升幅度：`3.75%` → `2.5%` （每 10 级差异：`44.5%` → `28%`）
  - 此调整横贯所有数值公式，因此在玩家对战对应等级的怪物中将**不会产生实质性的相对变化**。
  - 在这一调整后，等级间差异将变得稍缓，将能降低跨副本等场景的数值压力，并减轻装备的代际差异。
- 调整了「空庭」套装的套装效果，新增了减少技能冷却之效果。
- 新增了较多的随机套装，涵盖生存、辅助提升、攻击等方面。
  <br>目前已有：
  - 物理： `迅袭` `守望`
  - 近战：`重甲` `战甲` `狂战`
  - 弓：`猎手` `飞隼` `风行`
  - 魔法：`法袍` `奥术` `咒术` `灵织`
- 新增 `生命药剂`，于副本内通过击杀 Elite 或以上怪物概率产出。可受自然馈赠增益。
- 新增了怪物攻击相关的随机事件。
- 修复了此前掉落概率增益的异常偏移问题。
- 修复了谷垣 Elite 不掉落装备的问题。
- 重设了部分副属性词条，部分装备**可能会重新生成随机属性**，带来不便敬请谅解。
- 修复了版本更新时由熔铸带来的属性提升可能会被覆盖的问题。
- 修复了部分问题。
...
- 较大程度上降低了升阶台的 `晶髓碎片` 需求。（在 Lv.70 情况下为 -30%）
- 重新制作了部分交互方块的表现样式，使其更美观、更具特点。
- 修复了天赋「搜略本能」跨服CD不共享的问题。
- 修复了天赋「搜略本能」能重置副本内所有宝箱的问题。
- 新增了额外 `24` 个主城宝箱，并将主城宝箱的 CD 调整为 `40mins`
- 你现可通过更新副本的回忆点数升级较早副本的 `熟能生巧` 天赋。
  - 每级将固定需要 `6000` 对应难度副本回忆点数。
  - `熟能生巧` 天赋最终可升级的上限为 `Tier X` (+50)。
  - 该项等级额外限制为当前副本满级 `-5`，即在当前能打造 `Lv.70` 最新装备的情况下，将允许提升至 `Lv.65`。
- 你现可钓到灵性的鱼，且灵性鱼库也已投入使用（测试中）。
- 修复了部分细节问题。
...
- 提高了饰品的品质影响。
- 新增符文熔铸台，可用以将附属符文镶嵌至核心符文中。
- 对谷垣 `梦魇` 副本地图进行了优化。
- 将副本事件相关事务和副本进行了剥离，并进行了部分记录优化。
- 修复了多个问题。
...
- **回声雷鸣** 技能调整：
  ...
  - 伤害系数：100% → `160%`
  - 新增雷击特效。
  - 现在在使用后无可用目标时将**不会进入** CD。
  - 目标选择策略：优先选择 `8` 格内 **距离最近**的目标 → `6` 格内 **距离最远**的目标。
- 将伪装依赖更改为 Rapids 自研的轻量模块，解决了原依赖所带来的内存泄露、数据包异常增大及服务器运行时间过长无法进入等问题。
- **掉落机制** 修改：
  - `Essence`、`Final` 的掉落机制由零和互斥修改为**独立随机**，你现有可能同时掉落装备、饰品、图纸。
  - 所有特殊掉落现将受 <span class="item-name-tag">自然馈赠</span> 增益。
- **回忆解锁机制** 调整：
  - 基准期望获取值：`0.75%` → `0.9%`
  - 河隘倍率：`150%` → `175%` (综合提升：+40%)
- 你现可挑战新的 Final 怪物 `镇岳玄犀`。
- 「谷垣」`梦魇` 现已开放。
- 增强了 `空庭` 套装效果。
- 修复了部分问题。
...
- 新增扫荡功能，默认每日 `8` 次，每周额外提供 `level/10` 次用以冗余。Rank 附更多每日次数。
- 天赋 `搜略本能` 相关：
  - 现已修改为每副本独立 CD。
  - 超大幅降低了升级需求。
- 修复了部分问题。
...
- 进行了关于「谷垣」副本的部分精细化调整。
- <span class="item-name-tag">Metaphysician</span> 或以上用户将可在 Rank 菜单中使用 `拆解台`。
- 修复了部分问题。
...
- 修复了市场的部分问题。
- 完美修复了数字ID无法正常游玩的问题。
- 新增钓鱼自动出售功能，<span class="item-name-tag">Metaphysician</span> 或以上用户将可在 Rank 菜单中选择启用何种品质。
- 「谷垣」`踏察` `破晓` 现已开放。
- 新增了「谷垣」Tier 2 宝箱。
- 你现已可打造「空庭」系列装备。
- 修复了部分问题。
...
<div class="little-tips" style="max-width: 600px;">
  <div style="font-size:24px;">⛰</div>
  <div>
    <p><strong>谷垣</strong></p>
    <p style="color:#aaa; font-size: 13px; line-height: 1.35;">现已开放</span><br>
    谷垣将带来全新的怪物类型及更多样化的装备，你可尝试构建更符合自己意愿的配装。<br>
    你现可挑战难度「初涉」、「见闻」，并获得 Lv.41 ~ Lv.50 之装备。
    </p>
  </div>
</div>
...
- 完成了新副本「谷垣」：
  ...
  - 新的宝箱、收集物。
  - 完善了副本地图差分。
  - 由此副本起，你在「初涉」难度即可召唤历史回响。
- 新的装备类型：
  - `重甲`：战士装备。高物理抗性模型，主打生存，并附生命恢复等套装效果。
  - `猎手`：弓箭手装备。主打伤害，附暴击伤害等套装效果。
  - `法袍`：法师装备。主打续航，附最大法力值等套装效果。
  - ...还有 `9` 套常见套装类型将在后续版本陆续添加。
- 物品管理器支持了更完善的自定义装备类型、Tags 的传递。
- 近战攻击提供了更直观的攻击范围挥击特效。
- 支持了鱼竿相关属性。
- 修复了回忆获取速率始终保持为「峡谷」`初涉` 的高倍率的问题。
- 修复了部分问题。
...
- 调整了玩家数据的储存方式，修改为更安全的储存方式，并将部分重要数据更改为即刻入库。
- 修复了钓鱼的部分问题。
- 完成了新副本「谷垣」的部分地图修改。
...
- **河雾怨魄** 技能修改：
  - `雾步`:
    ...
    - 技能冷却：8s → `12s`。
    - 落点范围：2格 → `3格`。
    - `新增条件`：只有在 `80%` 血量以下才会释放。
    - 完整技能效果：
      - 释放后向附近 `30` 格内玩家发出Title警告，并播放警告声音。在 `1.25s` 后传送至目标位置 `3格` 附近。
- 修复了部分套装不能跨类型部件熔铸的问题。
- 修复了加载属性可能会触发死锁的问题。
- 修复了垃圾库移除物品异常的问题。
